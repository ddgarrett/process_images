    ...
    for row in range(THUMBNAILS_PER_PAGE[1]):
        ...
        for col in range(THUMBNAILS_PER_PAGE[0]):
            try:
                ...
                row_layout.append(sg.B('',k=(row,col), size=(0,0), pad=THUMBNAIL_PAD, button_color=("black","black"), ))
            except Exception as e:
                print(e)
                pass
        layout += [row_layout]
...
def main():
    sg.theme('black')
    ROOT_FOLDER = sg.popup_get_folder('Folder to display')
    if not ROOT_FOLDER:
        return
    
    files = [os.path.join(ROOT_FOLDER, f) for f in os.listdir(ROOT_FOLDER) if True in [f.lower().endswith(e) for e in EXTS]]
    files.sort()
